##h050_meta_analysis_unpaired_go_cc.r
##2016-01-04 fgarcia@cipf.es, antoi@alumni.uv.es
##Functional Meta-Analysis of transcriptomic studies and visualization
##Annotation: GO CC 
...
ficheros <- dir(pattern= "cc.RData")
ficheros

# we search a list including all unique GOs for all studies
gos<-NULL
for (fi in ficheros){
  load (fi)
  gos <- c(gos, rownames (res))
}
length (gos)
gos <- unique (gos)
length (gos)
...
mat.lor <- matrix (NA, nrow = length (gos), ncol = length (ficheros))
rownames (mat.lor) <- gos
...
mat.sd <- matrix (NA, nrow = length (gos), ncol = length (ficheros))
rownames (mat.sd) <- gos
...
mat.adjp <- matrix (NA, nrow = length (gos), ncol = length (ficheros))
rownames (mat.adjp) <- gos
...
for (i in methods){
  ...
  if (file.exists(file.path (.job$dir$code, "results",  "go_cc", "unpaired","files"))){
    setwd(file.path(.job$dir$code, "results",  "go_cc", "unpaired","files"))
  } else {
    dir.create(file.path(.job$dir$code, "results",  "go_cc", "unpaired","files"), recursive = T)
    setwd(file.path(.job$dir$code, "results",  "go_cc", "unpaired","files"))    
  }
  ...
  if (i == "DL"){
   ...
   #these lines, only for DL to include in final report
    ...
    sgos <- rownames(DLs_or)
    color <- rep("red", length(sgos))
    color[DLs_or$summary_LOR < 0] <- "green"
    pngRes <- getAmigoTree(goIDs=sgos, color=color, filename= "sig.gograph.png", 
  ...
  if (i == "SJ"){
    SJ <- result_meta
    SJ[,"name"] <- getGOnames(rownames(SJ))
    SJ[, "ID"]   <- rownames(SJ)
  ...
  #   if (i == "ML"){
  #     ML <- result_meta
  #     ML[,"name"] <- getGOnames(rownames(ML))
  #     ML[, "ID"]   <- rownames(ML)
  ...
  #   if (i == "REML"){
  #     REML <- result_meta
  #     REML[,"name"] <- getGOnames(rownames(REML))
  ...
  #   if (i == "EB"){
  #     EB <- result_meta
  #     EB[,"name"] <- getGOnames(rownames(EB))
  #     EB[, "ID"]   <- rownames(EB)
  ...
  if (i == "PM"){
    PM <- result_meta
    PM[,"name"] <- getGOnames(rownames(PM))
    PM[, "ID"]   <- rownames(PM)
...
if (file.exists(file.path (.job$dir$code, "results",  "go_cc", "unpaired", "plots"))){
  setwd(file.path(.job$dir$code, "results",  "go_cc", "unpaired", "plots"))
} else {
  dir.create(file.path(.job$dir$code, "results",  "go_cc", "unpaired", "plots"), recursive = T)
  setwd(file.path(.job$dir$code, "results",  "go_cc", "unpaired", "plots"))    
}
...
## 3.1. plots to explore INPUT data: LOR and SD matrix 
######################################################
#These plots are the same for any method. 
#That changes when having different biological database 
#because we have different results from logistic model

gmat.sd  <- mat.sd
colnames(gmat.sd) <-  toupper(substr(colnames(mat.sd),1,4))
g2mat.sd <- melt(gmat.sd)
colnames(g2mat.sd) <- c("function", "study", "sd")
head(g2mat.sd); dim(g2mat.sd)

gmat.lor  <- mat.lor
colnames(gmat.lor) <-  toupper(substr(colnames(mat.lor),1,4))
...
jpeg (filename = "go_cc_unpaired_tcga_sd.jpeg",  
...
jpeg (filename = "go_cc_unpaired_tcga_lor.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
x.por <- 3
y.por <- 3
jpeg (filename = "volcano_unpaired_go_cc.jpeg",  width = 480 *x.por ,   height = 480*y.por, pointsize = 100, quality = 100) 
...
jpeg (filename = "go_cc_het_QEp.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
for (i in 2:length(param)){
  jpeg (filename = paste("go_cc_het_", param[i], ".jpeg", sep = ""),  
        width = 480 *x.por ,   height = 480*y.por,
        pointsize = 150, quality = 100) 
...
if (length(sig.fun) == 0){print ("Not significant results")} else {
  for (i in 1:length(sig.fun)){
    ...
    #   jpeg (filename = paste("go_cc_",sig.fun[i],".png",sep =""),
    #         width = 480 *x.por ,   height = 480*y.por, pointsize = 100, quality = 100) 
    
    png (filename = paste("CC_forest_", substr(sig.fun[i],4,10),".png",sep =""),  
    ...
    png (filename = paste("CC_funnel_",substr(sig.fun[i],4,10),".png",sep =""),  
         width = 480 * x.por, height = 480 * y.por, res = 200)
    ...
    #     png (filename = paste("go_cc_funnel_tf_",sig.fun[i],".png",sep =""),
    #          width = 480 * x.por, height = 480 * y.por, res = 200)
    #     par(mar=c(5,4,1,2))
    ...
    ### to save as png file
    png (filename = paste("CC_radial_",substr(sig.fun[i],4,10),".png",sep =""),  
         width = 480 * x.por, height = 480 * y.por, res = 200)
    ...
    ### to save as png file
#     jpeg (filename = paste("CC_influ_",substr(sig.fun[i],4,10),".jpeg",sep =""),  
#           width = 480 *x.por ,   height = 480*y.por,
#           pointsize = 50, quality = 100) 
    png (filename = paste("CC_influ_",substr(sig.fun[i],4,10),".png",sep =""),  
         width = 480 * x.por, height = 480 * y.por, res = 200)
    inf <- influence(res)
    plot(inf, plotfb = T)
    dev.off()
...
mat.sen <- mat.sen[, c("DL", "HE", "HS", "SJ", "PM")]
...
jpeg (filename = "go_cc_unpaired_sens_allmethods.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
if (length(sig.fun) == 0){print ("Not significant results")} else {
  for (i in 1:length(sig.fun)){
    ...
    write.table(resu.l1out, 
                paste("sensi_unpaired_cc_", substr(sig.fun[i],4,10),".txt", sep =""),
                quote = F, row.names = F)
  }
}
...
# ## 3.6. Visualization of significant GOs together 
# ###################################################### 
# 
...
# pngRes <- getAmigoTree(goIDs=GO_over, color=color, filename= "sig_go_bp_over.png", 
#                        picType="png", saveResult=TRUE)
# 
# 
...
# pngRes <- getAmigoTree(goIDs=GO_under, color=color, filename= "sig_go_bp_under.png", 
#                        picType="png", saveResult=TRUE)
# 
# 
# #example:
# gos <- c("GO:0030259", "GO:0044255", "GO:0044723", "GO:0006629")
# length(gos)
# color <- rep("green", length(gos))
# pngRes <- getAmigoTree(goIDs=gos, color=color, filename= "sig_gos.png", 
#                        picType="png", saveResult=TRUE)
# 
# jpeg (filename = paste("go_cc_het_", param[i], ".jpeg", sep = ""),  
#       width = 480 *x.por ,   height = 480*y.por,
...
# ## RADIAL PLOTS comparing two methods for GO:0016922
# # Radial plots were introduced by Rex Galbraith (1988a, 1988b, 1994) 
# # and can be useful in the meta-analytic context to examine the data for heterogeneity.
# # http://www.metafor-project.org/doku.php/plots:radial_plot
# 
...
# x.por = 2.5; y.por = 2.5
# 
# res1 <- rma(yi= mat.lor["GO:0001105",], sei =mat.sd["GO:0001105",],
#            method = "DL")
# res2 <- rma(yi= mat.lor["GO:0001105",], sei =mat.sd["GO:0001105",],
#             method = "FE")
# res1
# res2
# 
# 
# ### to save as png file
# png (filename = paste("cc_two_radial_",substr("GO:0001105",4,10),".png",sep =""),  
#      width = 480 * x.por, height = 480 * y.por, res = 200)
# ### adjust margins so the space is better used
# par(mfrow=c(2,1))
# # par(mar=c(5,4,0,2))
# radial(res1,back ="darkslategray1", main = "Modelo de efectos aleatorios", cex =0.7)
# radial(res2,back ="darkslategray1", main = "Modelo de efectos fijos", cex = 0.7)
# par(mfrow=c(1,1))
# dev.off()
# 
# 
