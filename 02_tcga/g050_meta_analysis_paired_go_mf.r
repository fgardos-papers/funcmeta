##g050_meta_analysis_paired_go_mf.r
...
for (i in methods){
  ...
  if (file.exists(file.path (.job$dir$code, "results",  "go_mf", "paired","files"))){
    setwd(file.path(.job$dir$code, "results",  "go_mf", "paired","files"))
  } else {
    dir.create(file.path(.job$dir$code, "results",  "go_mf", "paired","files"), recursive = T)
    setwd(file.path(.job$dir$code, "results",  "go_mf", "paired","files"))    
  }
...
if (file.exists(file.path (.job$dir$code, "results",  "go_mf", "paired", "plots"))){
  setwd(file.path(.job$dir$code, "results",  "go_mf", "paired", "plots"))
} else {
  dir.create(file.path(.job$dir$code, "results",  "go_mf", "paired", "plots"), recursive = T)
  setwd(file.path(.job$dir$code, "results",  "go_mf", "paired", "plots"))    
}
...
jpeg (filename = "go_mf_paired_tcga_sd.jpeg",  
...
jpeg (filename = "go_mf_paired_tcga_lor.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
x.por <- 3
y.por <- 3
jpeg (filename = "volcano_paired_go_mf.jpeg",  width = 480 *x.por ,   height = 480*y.por, pointsize = 100, quality = 100) 
...
jpeg (filename = "go_mf_paired_sens_allmethods.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
if (length(sig.fun) == 0){print ("Not significant results")} else {
  for (i in 1:length(sig.fun)){
    ...
    write.table(resu.l1out, 
                paste("sensi_paired_mf_", substr(sig.fun[i],4,10),".txt", sep =""),
                quote = F, row.names = F)
  }
}
