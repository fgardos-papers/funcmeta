##h050_meta_analysis_unpaired_reactome.r
...
for (i in methods){
  ...
  if (file.exists(file.path (.job$dir$code, "results",  "reactome", "unpaired","files"))){
    setwd(file.path(.job$dir$code, "results",  "reactome", "unpaired","files"))
  } else {
    dir.create(file.path(.job$dir$code, "results",  "reactome", "unpaired","files"), recursive = T)
    setwd(file.path(.job$dir$code, "results",  "reactome", "unpaired","files"))    
  }
...
if (file.exists(file.path (.job$dir$code, "results",  "reactome", "unpaired", "plots"))){
  setwd(file.path(.job$dir$code, "results",  "reactome", "unpaired", "plots"))
} else {
  dir.create(file.path(.job$dir$code, "results",  "reactome", "unpaired", "plots"), recursive = T)
  setwd(file.path(.job$dir$code, "results",  "reactome", "unpaired", "plots"))    
}
...
x.por <- 3
y.por <- 3
jpeg (filename = "volcano_unpaired_reactome.jpeg",  width = 480 *x.por ,   height = 480*y.por, pointsize = 100, quality = 100) 
...
jpeg (filename = "reactome_unpaired_sens_allmethods.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
if (length(sig.fun) == 0){print ("Not significant results")} else {
  for (i in 1:length(sig.fun)){
    ...
    write.table(resu.l1out, 
                paste("sensi_unpaired_reactome_", sig.fun[i],".txt", sep =""),
                quote = F, row.names = F)
  }
}
