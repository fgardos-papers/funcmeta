##g050_meta_analysis_paired_go_bp.r
...
for (i in methods){
  ...
  if (file.exists(file.path (.job$dir$code, "results",  "go_bp", "paired","files"))){
    setwd(file.path(.job$dir$code, "results",  "go_bp", "paired","files"))
  } else {
    dir.create(file.path(.job$dir$code, "results",  "go_bp", "paired","files"), recursive = T)
    setwd(file.path(.job$dir$code, "results",  "go_bp", "paired","files"))    
  }
...
if (file.exists(file.path (.job$dir$code, "results",  "go_bp", "paired", "plots"))){
  setwd(file.path(.job$dir$code, "results",  "go_bp", "paired", "plots"))
} else {
  dir.create(file.path(.job$dir$code, "results",  "go_bp", "paired", "plots"), recursive = T)
  setwd(file.path(.job$dir$code, "results",  "go_bp", "paired", "plots"))    
}
...
jpeg (filename = "go_bp_paired_tcga_sd.jpeg",  
...
jpeg (filename = "go_bp_paired_tcga_lor.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
x.por <- 3
y.por <- 3
jpeg (filename = "volcano_paired_go_bp.jpeg",  width = 480 *x.por ,   height = 480*y.por, pointsize = 100, quality = 100) 
...
jpeg (filename = "go_bp_paired_sens_allmethods.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
if (length(sig.fun) == 0){print ("Not significant results")} else {
  for (i in 1:length(sig.fun)){
    ...
    write.table(resu.l1out, 
                paste("sensi_paired_bp_", substr(sig.fun[i],4,10),".txt", sep =""),
                quote = F, row.names = F)
  }
}
