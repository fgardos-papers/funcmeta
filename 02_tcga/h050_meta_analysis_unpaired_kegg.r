...
# we search a list including all unique kegg IDs for all studies
kegguniq<-NULL
for (fi in ficheros){
  load (fi)
  kegguniq <- c(kegguniq, rownames (res))
}
length (kegguniq)
kegguniq <- unique (kegguniq)
length (kegguniq)
kegguniq <- sort (kegguniq)


### generating matrix with all LOR for all studies
mat.lor <- matrix (NA, nrow = length (kegguniq), ncol = length (ficheros))
rownames (mat.lor) <- kegguniq
...
mat.sd <- matrix (NA, nrow = length (kegguniq), ncol = length (ficheros))
rownames (mat.sd) <- kegguniq
...
mat.adjp <- matrix (NA, nrow = length (kegguniq), ncol = length (ficheros))
rownames (mat.adjp) <- kegguniq
...
for (i in methods){
  ...
  if (file.exists(file.path (.job$dir$code, "results",  "kegg", "unpaired","files"))){
    setwd(file.path(.job$dir$code, "results",  "kegg", "unpaired","files"))
  } else {
    dir.create(file.path(.job$dir$code, "results",  "kegg", "unpaired","files"), recursive = T)
    setwd(file.path(.job$dir$code, "results",  "kegg", "unpaired","files"))    
  }
...
if (file.exists(file.path (.job$dir$code, "results",  "kegg", "unpaired", "plots"))){
  setwd(file.path(.job$dir$code, "results",  "kegg", "unpaired", "plots"))
} else {
  dir.create(file.path(.job$dir$code, "results",  "kegg", "unpaired", "plots"), recursive = T)
  setwd(file.path(.job$dir$code, "results",  "kegg", "unpaired", "plots"))    
}
...
x.por <- 3
y.por <- 3
jpeg (filename = "volcano_unpaired_kegg.jpeg",  width = 480 *x.por ,   height = 480*y.por, pointsize = 100, quality = 100) 
...
if (length(sig.fun) == 0){print ("Not significant results")} else {
  for (i in 1:length(sig.fun)){
    ...
    #   jpeg (filename = paste("kegg_",sig.fun[i],".png",sep =""),
    #         width = 480 *x.por ,   height = 480*y.por, pointsize = 100, quality = 100) 
    
    png (filename = paste("kegg_forest_", sig.fun[i],".png",sep =""),  
    ...
    png (filename = paste("kegg_funnel_",sig.fun[i],".png",sep =""),  
         width = 480 * x.por, height = 480 * y.por, res = 200)
    ...
    ### to save as png file
    png (filename = paste("kegg_radial_",sig.fun[i],".png",sep =""),  
         width = 480 * x.por, height = 480 * y.por, res = 200)
    ...
    ### to save as png file
#     jpeg (filename = paste("kegg_influ_",sig.fun[i],".jpeg",sep =""),  
#           width = 480 *x.por ,   height = 480*y.por,
#           pointsize = 50, quality = 100) 
    png (filename = paste("kegg_influ_",sig.fun[i],".png",sep =""),  
         width = 480 * x.por, height = 480 * y.por, res = 200)
    inf <- influence(res)
    plot(inf, plotfb = T)
    dev.off()
...
jpeg (filename = "kegg_unpaired_sens_allmethods.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
if (length(sig.fun) == 0){print ("Not significant results")} else {
  for (i in 1:length(sig.fun)){
    ...
    write.table(resu.l1out, 
                paste("sensi_unpaired_kegg_", sig.fun[i],".txt", sep =""),
                quote = F, row.names = F)
  }
}
...
## 3.2. diagnostic plots for a specific model 
###################################################### 
...
#     # This funnel plot helps to detect asymmetries
#     # http://www.metafor-project.org/doku.php/plots:funnel_plot_with_trim_and_fill
#     png (filename = paste("KEGG_funnel_tf_",sig.fun[i],".png",sep =""),
#          width = 480 * x.por, height = 480 * y.por, res = 200)
#     par(mar=c(5,4,1,2))
#     ### carry out trim-and-fill analysis
#     taf <- trimfill(res)
