...
for (i in methods){
  ...
  if (file.exists(file.path (.job$dir$code, "results",  "kegg", "paired","files"))){
    setwd(file.path(.job$dir$code, "results",  "kegg", "paired","files"))
  } else {
    dir.create(file.path(.job$dir$code, "results",  "kegg", "paired","files"), recursive = T)
    setwd(file.path(.job$dir$code, "results",  "kegg", "paired","files"))    
  }
...
if (file.exists(file.path (.job$dir$code, "results",  "kegg", "paired", "plots"))){
  setwd(file.path(.job$dir$code, "results",  "kegg", "paired", "plots"))
} else {
  dir.create(file.path(.job$dir$code, "results",  "kegg", "paired", "plots"), recursive = T)
  setwd(file.path(.job$dir$code, "results",  "kegg", "paired", "plots"))    
}
...
x.por <- 3
y.por <- 3
jpeg (filename = "volcano_paired_kegg.jpeg",  width = 480 *x.por ,   height = 480*y.por, pointsize = 100, quality = 100) 
...
jpeg (filename = "kegg_paired_sens_allmethods.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
if (length(sig.fun) == 0){print ("Not significant results")} else {
  for (i in 1:length(sig.fun)){
    ...
    write.table(resu.l1out, 
                paste("sensi_paired_kegg_", sig.fun[i],".txt", sep =""),
                quote = F, row.names = F)
  }
}
