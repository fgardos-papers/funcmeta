##h050_meta_analysis_unpaired_go_mf.r
...
for (i in methods){
  ...
  if (file.exists(file.path (.job$dir$code, "results",  "go_mf", "unpaired","files"))){
    setwd(file.path(.job$dir$code, "results",  "go_mf", "unpaired","files"))
  } else {
    dir.create(file.path(.job$dir$code, "results",  "go_mf", "unpaired","files"), recursive = T)
    setwd(file.path(.job$dir$code, "results",  "go_mf", "unpaired","files"))    
  }
...
if (file.exists(file.path (.job$dir$code, "results",  "go_mf", "unpaired", "plots"))){
  setwd(file.path(.job$dir$code, "results",  "go_mf", "unpaired", "plots"))
} else {
  dir.create(file.path(.job$dir$code, "results",  "go_mf", "unpaired", "plots"), recursive = T)
  setwd(file.path(.job$dir$code, "results",  "go_mf", "unpaired", "plots"))    
}
...
jpeg (filename = "go_mf_unpaired_tcga_sd.jpeg",  
...
jpeg (filename = "go_mf_unpaired_tcga_lor.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
x.por <- 3
y.por <- 3
jpeg (filename = "volcano_unpaired_go_mf.jpeg",  width = 480 *x.por ,   height = 480*y.por, pointsize = 100, quality = 100) 
...
jpeg (filename = "go_mf_het_QEp.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
for (i in 2:length(param)){
  jpeg (filename = paste("go_mf_het_", param[i], ".jpeg", sep = ""),  
        width = 480 *x.por ,   height = 480*y.por,
        pointsize = 150, quality = 100) 
...
if (length(sig.fun) == 0){print ("Not significant results")} else {
  for (i in 1:length(sig.fun)){
    ...
    #   jpeg (filename = paste("go_mf_",sig.fun[i],".png",sep =""),
    #         width = 480 *x.por ,   height = 480*y.por, pointsize = 100, quality = 100) 
    
    png (filename = paste("MF_forest_", substr(sig.fun[i],4,10),".png",sep =""),  
    ...
    png (filename = paste("MF_funnel_",substr(sig.fun[i],4,10),".png",sep =""),  
         width = 480 * x.por, height = 480 * y.por, res = 200)
    ...
    ### to save as png file
    png (filename = paste("MF_radial_",substr(sig.fun[i],4,10),".png",sep =""),  
         width = 480 * x.por, height = 480 * y.por, res = 200)
    ...
    ### to save as png file
#     jpeg (filename = paste("MF_influ_",substr(sig.fun[i],4,10),".jpeg",sep =""),  
#           width = 480 *x.por ,   height = 480*y.por,
#           pointsize = 50, quality = 100) 
    png (filename = paste("MF_influ_",substr(sig.fun[i],4,10),".png",sep =""),  
         width = 480 * x.por, height = 480 * y.por, res = 200)
    inf <- influence(res)
    plot(inf, plotfb = T)
    dev.off()
...
jpeg (filename = "go_mf_unpaired_sens_allmethods.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
if (length(sig.fun) == 0){print ("Not significant results")} else {
  for (i in 1:length(sig.fun)){
    ...
    write.table(resu.l1out, 
                paste("sensi_unpaired_mf_", substr(sig.fun[i],4,10),".txt", sep =""),
                quote = F, row.names = F)
  }
}
...
# 
# ### to save as png file
# png (filename = paste("mf_two_radial_",substr("GO:0001105",4,10),".png",sep =""),  
#      width = 480 * x.por, height = 480 * y.por, res = 200)
# ### adjust margins so the space is better used
# par(mfrow=c(2,1))
# # par(mar=c(5,4,0,2))
