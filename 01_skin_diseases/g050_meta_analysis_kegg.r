...
setwd (file.path (.job$dir$proces, "gsa", "kegg"))
...
for (i in methods){
  ...
  if (file.exists(file.path (.job$dir$code, "results",  "kegg", "files"))){
    setwd(file.path(.job$dir$code, "results",  "kegg", "files"))
  } else {
    dir.create(file.path(.job$dir$code, "results",  "kegg", "files"), recursive = T)
    setwd(file.path(.job$dir$code, "results",  "kegg", "files"))    
  }
...
if (file.exists(file.path (.job$dir$code, "results",  "kegg",  "plots"))){
  setwd(file.path(.job$dir$code, "results",  "kegg",  "plots"))
} else {
  dir.create(file.path(.job$dir$code, "results",  "kegg",  "plots"), recursive = T)
  setwd(file.path(.job$dir$code, "results",  "kegg",  "plots"))    
}
...
x.por <- 3
y.por <- 3
jpeg (filename = "volcano_kegg.jpeg",  width = 480 *x.por ,   height = 480*y.por, pointsize = 100, quality = 100) 
...
jpeg (filename = "kegg_sens_allmethods.jpeg",  
      width = 480 *x.por ,   height = 480*y.por,
      pointsize = 150, quality = 100) 
...
#PROBLEMAS PARA N = 26 con la dimensión????
setwd(file.path(.job$dir$code, "results",  "kegg",  "files"))    
...
if (length(sig.fun) == 0){print ("Not significant results")} else {
  for (i in 1:length(sig.fun)){
    ...
    studies <- colnames(mat.lor)
    resu.l1out <- cbind(studies,resu.l1out)
    #head(resu.l1out)
    write.table(resu.l1out, 
                paste("sensi_kegg_", sig.fun[i],".txt", sep =""),
                quote = F, row.names = F)
  }
}
